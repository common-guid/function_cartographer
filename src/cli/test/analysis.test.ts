...
describe('Analysis Logic', () => {
  ...
  describe('Analyzer', () => {
    it('processDirectory handles empty directory', async () => {
       ...
       if (!result.success) {
           expect((result as any).error).toContain('No JS bundles found')
       }
    })
    ...
    it('postProcess: cached output -> read output -> move input to dupes', async () => {
         ...
         const dupesDir = path.resolve('/dupes')
          ...
          mockReadFile.mockImplementation((p) => {
              if (p.includes('output')) return Promise.resolve('CACHED_CONTENT')
     ...
     it('postProcess: vendor file skipped by humanify -> fallback to beautify', async () => {
         ...
         mockReaddir.mockImplementation((dir) => {
             if (dir === inputDir) return Promise.resolve([{ name: 'node_modules', isDirectory: () => true, isFile: () => false }])
             if (dir.endsWith('node_modules')) return Promise.resolve([{ name: 'vendor.js', isDirectory: () => false, isFile: () => true }])
         ...
         expect(mockWriteFile).toHaveBeenCalledWith(
             ...
             expect.stringContaining('/* beautified */')
         )
     })
  })
})
